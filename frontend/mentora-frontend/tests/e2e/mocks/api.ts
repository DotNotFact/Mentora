...
function toEnrollmentWithCourse(db: MockDb, enrollmentId: string) {
  ...
  return { ...enrollment, course: withRating(db, course) };
}
...
// averageRating/reviewsCount не хранятся на MockCourse — вычисляются из
// db.reviews при каждом ответе, единственный источник истины — сами
// отзывы (иначе легко рассинхронизировать при добавлении/редактировании).
function withRating(db: MockDb, course: MockCourse) {
  const reviews = db.reviews[course.id] ?? [];
  const reviewsCount = reviews.length;
  const averageRating =
    reviewsCount === 0
      ? null
      : Math.round((reviews.reduce((sum, r) => sum + r.rating, 0) / reviewsCount) * 10) / 10;
  return { ...course, averageRating, reviewsCount };
}
  ...
  await page.route(
    ...
    async (route: Route) => {
      ...
      if (method === 'GET' && path === '/courses') {
        ...
        if (sort === 'rating') {
          rated = [...rated].sort((a, b) => (b.averageRating ?? -1) - (a.averageRating ?? -1));
        }
        return json(200, { items: rated, total: rated.length, page: 1, pageSize: 12 });
      }
      ...
      if (courseMatch) {
        ...
        if (method === 'GET') {
          ...
          return course ? json(200, withRating(db, course)) : json(404, { message: 'Not found' });
        }
        if (method === 'PUT') {
          ...
          return json(200, withRating(db, db.courses[index]));
        }
      }

      // --- reviews ---
      const reviewsMatch = path.match(/^\/courses\/([^/]+)\/reviews$/);
      if (reviewsMatch) {
        const courseId = reviewsMatch[1];
        if (method === 'GET') {
          return json(200, db.reviews[courseId] ?? []);
        }
        if (method === 'POST') {
          if (!userId) return json(401, { message: 'Unauthorized' });
          ...
          if (!isEnrolled) return json(403, { message: 'Not enrolled' });
          const author = db.users.find((u) => u.id === userId);
          const { rating, comment } = body as { rating: number; comment?: string | null };
          const entries = (db.reviews[courseId] ??= []);
          ...
          const review = {
            ...
            courseId,
            userId,
            userName: author?.fullName ?? 'Студент',
            rating,
      ...
      const reviewUpdateMatch = path.match(/^\/courses\/([^/]+)\/reviews\/([^/]+)$/);
      if (reviewUpdateMatch && method === 'PUT') {
        if (!userId) return json(401, { message: 'Unauthorized' });
        const [, courseId, reviewId] = reviewUpdateMatch;
        const entries = db.reviews[courseId] ?? [];
        const review = entries.find((r) => r.id === reviewId && r.userId === userId);
        if (!review) return json(404, { message: 'Not found' });
        const { rating, comment } = body as { rating: number; comment?: string | null };
      ...
      const lessonProgressMatch = path.match(
        /^\/enrollments\/([^/]+)\/lessons\/([^/]+)\/progress$/,
      );
      if (lessonProgressMatch && method === 'PUT') {
        if (!userId) return json(401, { message: 'Unauthorized' });
        const [, courseId, lessonId] = lessonProgressMatch;
        const { positionSeconds, completed } = body as {
